...
def main():
    ...
    print(f"1. 堆芯冷却剂出口温度:     {T_out_avg:.2f} ℃")
    
    print(f"2. 燃料棒表面平均热流密度: {q_flux_avg/1e6:.3f} MW/m^2")
    print(f"   燃料棒表面最大热流密度: {q_flux_max/1e6:.3f} MW/m^2")
    print(f"   平均线功率:             {q_L_avg/1000:.2f} kW/m")
    print(f"   最大线功率:             {q_L_max/1000:.2f} kW/m")
    
    print("3. 热管的焓、包壳内外表面温度、芯块中心温度随轴向的分布: (详见生成图表1和图表4)")
    
    print(f"4. 包壳表面最高温度:       {np.max(T_co_arr):.1f} ℃")
    print(f"   芯块中心最高温度:       {np.max(T_0_arr):.1f} ℃")
    
    print(f"5. DNBR在轴向上的变化:     (详见生成图表3), 最小DNBR值为 {np.min(DNBR_arr):.2f}")
    
    print(f"6. 计算堆芯压降:           {dp_avg_total/1000:.2f} kPa")
    print("="*60)
